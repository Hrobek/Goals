...
- [x] Siri fráze nad rámec AppShortcuts ("zaloguj 5 km běh"). Hotovo: `HabitEntity`/`GoalEntity` + intenty "Zapsat návyk" a "Zapsat pokrok u cíle" (volba Splnit / Přidat množství), "Co dnes zbývá", "Otevřít návyk/cíl" v `Goals/Shortcuts/`. Zkratka "Refresh widgets" odstraněna. Spotlight: `SpotlightIndexer` indexuje návyky a cíle (s emoji náhledem), ťuknutí otevře detail. Další krok: výstupy pro automatizace, Control Center control.
